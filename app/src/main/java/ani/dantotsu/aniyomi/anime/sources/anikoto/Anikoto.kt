...
class Anikoto(
    ...
    private suspend fun extractFromPlayer(
        ...
        val dataId = DATA_ID_REGEX.find(pageBody)?.groupValues?.get(1)
            ?: EMBED_PATH_ID_REGEX.find(embedUrl)?.groupValues?.get(1)
        if (dataId != null) {
    ...
    private fun parseM3u8FromSources(jsonStr: String): Pair<String, List<Track>> {
        ...
        val m3u8 = extractM3u8Url(obj)
        val subtitles = extractSubtitleTracks(obj)
        return Pair(m3u8, subtitles)
    }

    private fun extractM3u8Url(obj: JSONObject): String {
        val enc = obj.optString("enc")
        if (enc.isNotEmpty()) {
            val decryptedUrl = decryptEncryptedSource(enc)
            if (decryptedUrl.isNotEmpty()) {
                return signCdnToken(decryptedUrl)
            }
        }

        val sources = obj.opt("sources")
        if (sources is String && sources.startsWith("http")) return signCdnToken(sources)
        if (sources is JSONObject) {
            sources.optString("file").takeIf { it.isNotEmpty() }?.let { return signCdnToken(it) }
        }
        if (sources is JSONArray) {
            for (i in 0 until sources.length()) {
                ...
                if (item is JSONObject) {
                    ...
                    if (f.isNotEmpty()) return signCdnToken(f)
                }
                if (item is String && item.startsWith("http")) return signCdnToken(item)
            }
        }
        throw Exception("No valid m3u8 found in sources")
    }

    private fun decryptEncryptedSource(enc: String): String {
        return try {
            val keyBytes = ByteArray(32)
            val rawKey = MEGAPLAY_AES_KEY.toByteArray(Charsets.UTF_8)
            System.arraycopy(rawKey, 0, keyBytes, 0, minOf(32, rawKey.size))
            val secretKey = SecretKeySpec(keyBytes, "AES")
            val iv = IvParameterSpec(MEGAPLAY_AES_IV.toByteArray(Charsets.UTF_8))

            val decoded = Base64.decode(enc, Base64.URL_SAFE or Base64.DEFAULT)
            ...
            Log.e("Anikoto", "Failed to decrypt video source: ${e.message}")
            ""
        }
    }

    private fun signCdnToken(url: String): String {
        if (url.isEmpty() || url.contains("token=")) return url
        val match = CDN_PATH_REGEX.find(url) ?: return url
        return try {
            ...
            val keySpec = SecretKeySpec(MEGAPLAY_CDN_KEY.toByteArray(Charsets.UTF_8), "HmacSHA256")
            hmac.init(keySpec)
            val signature = hmac.doFinal(payload.toByteArray(Charsets.UTF_8))
            val payloadB64 = Base64.encodeToString(payload.toByteArray(Charsets.UTF_8), Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING).trimEnd('=')
            val sigB64 = Base64.encodeToString(signature, Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING).trimEnd('=')
            val token = "$payloadB64.$sigB64"
            val separator = if (url.contains("?")) "&" else "?"
            "$url${separator}token=$token"
        } catch (e: Exception) {
            Log.e("Anikoto", "Failed to sign CDN token: ${e.message}")
            url
        }
    }

    private fun extractSubtitleTracks(obj: JSONObject): List<Track> {
        ...
        if (subtitlesRaw is JSONArray) {
            for (i in 0 until subtitlesRaw.length()) {
                ...
                val label = sub.optString("label").ifEmpty { sub.optString("language", "Subtitle") }.ifEmpty { "Subtitle" }
    ...
    private fun extractFromHls(
        ...
        return try {
            val signedM3u8 = signCdnToken(m3u8Url)
            val request = GET(signedM3u8, vidHeaders)
                ...
                if (line.startsWith("#EXT-X-STREAM-INF:")) {
                    ...
                    val variantUrl = if (i + 1 < lines.size) {
                        ...
                        if (urlLine.isNotEmpty() && !urlLine.startsWith("#")) {
                            val resolved = resolveUrl(urlLine, signedM3u8)
                            signCdnToken(resolved)
                        } else {
                            null
                        }
                    } else null
            ...
            if (videos.isEmpty()) {
                videos.add(Video(signedM3u8, "$displayName - Auto", signedM3u8, vidHeaders, subtitleTracks = subtitleList))
            }
    ...
    companion object {
        ...
        private val EMBED_PATH_ID_REGEX = Regex("""/(?:embed|stream/[^/]+)/(\d+)""")
        private val CDN_PATH_REGEX = Regex("""/([a-f0-9]{32})/([a-f0-9]{32})/""", RegexOption.IGNORE_CASE)
        private const val MEGAPLAY_AES_KEY = "i?LMTAx0Q6,:}50U"
        private const val MEGAPLAY_AES_IV = "W0;27ToaUpl_P%'c"
        private const val MEGAPLAY_CDN_KEY = "MpCdnT0k3n!9f2K#xQ7vL5mR8wN1pY4s"
